...
func (db *StakeDatabase) WaitForHeight(height int64) chan *chainhash.Hash {
	...
	if dbHeight > height {
		log.Infof("11111111111 Rescanning  at dbHeight %d, height %d.", dbHeight, height)
		defer func() { waitChan <- nil }()
		return waitChan
	} else if dbHeight == height {
		log.Infof("222222222222 Rescanning  at dbHeight %d, height %d.", dbHeight, height)
	...
	log.Infof("3333333333333 Rescanning  at dbHeight %d, height %d.", dbHeight, height)
	db.waitMtx.Lock()
...
func (db *StakeDatabase) Open(dbName string) error {
	...
	err = db.StakeDB.View(func(dbTx database.Tx) error {
		hash, height, err := stake.FetchBestState(dbTx)
		if err != nil {
			return err;
		}
...
*/
		var errLocal error
		msgBlock, errLocal := db.NodeClient.GetBlock(hash)
		if errLocal != nil {
			return fmt.Errorf("GetBlock failed (%s): %v", *hash, errLocal)
		}
		header := msgBlock.Header

		db.BestNode, errLocal = stake.LoadBestNode(dbTx, height,
			*hash, header, db.params)
		return errLocal
	})
	if err != nil {
		if !isFreshDB {
