...
func (db *ChainDB) SyncChainDB(ctx context.Context, client rpcutils.MasterBlockGetter,
	...
	for ib := startHeight; ib <= nodeHeight; ib++ {
		log.Infof("5555555555555 ib %d.", ib)
...
/*		size := db.stakeDB.PoolSize()
		if size == 0 {
			db.stakeDB.ConnectBlockHash(blockHash)
		}
*/
	//	fmt.Println(size)
		...
		if tpi == nil{
			continue
			//return ib - 1, nil//fmt.Errorf("stakeDB.PoolInfo could not locate block %s", blockHash.String())
		}
		winners := tpi.Winners
